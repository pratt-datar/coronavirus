...
# QUERY
tweet_obj = tweets.find({"$or":[{"retweeted_status.extended_tweet.entities.hashtags.text":{"$exists":True}},{"entities.hashtags.text":{"$exists":True}}]},{"user.screen_name":1,"entities.hashtags.text":1,"retweeted_status.extended_tweet.entities.hashtags.text":1,"timestamp_ms":1,"_id":0})
...
with open('tweets.csv', 'w') as outfile:
	fields = ['screen_name','timestamp','rt_hashtags','hashtags']
	write = csv.DictWriter(outfile, fieldnames=fields)
	write.writeheader()
	for records in tweet_obj:  # Here we are using 'tweet_obj' as an iterator
		...
		try:
			for y in records['entities']['hashtags']:
				text_hashtags.append(y['text'])
		except:
			text_hashtags = None
		flattened_record = {'screen_name':username,'timestamp': timestamp, 'rt_hashtags': text_rt, 'hashtags': text_hashtags}
		write.writerow(flattened_record)
